...
class RxActivityTest {
    @get: Rule
    val mScenario = ActivityScenarioRule(RxActivity::class.java)
    ...
    fun testSearchChange() {
        Espresso.onView(ViewMatchers.withId(R.id.search))
            .perform(
                ViewActions.typeText("name 11")
            )
        Espresso.onView(ViewMatchers.withId(R.id.rv))
            .perform(
                RecyclerViewActions.scrollTo<RecyclerView.ViewHolder>(
                    ViewMatchers.hasDescendant(ViewMatchers.withText("name 11"))
                )
            )
    }
